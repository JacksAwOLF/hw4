...
Triangle::surfaceNormal(Arr3 point){
    return n;
}

Sphere::surfaceNormal(Arr3 point){
    return point - center;
}
...
float Triangle::intersectWithRay(Ray ray) {
    ...
    float beta = (pp * aap) / (bb * aap);
    float alpha = ((pp * n_aa) - (bb * n_aa) * beta) / aa.length();
...
float Sphere::intersectWithRay(Ray ray) {
    ...
    float a = ray.slope * ray.slope,
        b = ray.slope * (float)2 * (ray.start - center),
        c = (ray.start - center) * 
            (ray.start - center) - radius * radius;
